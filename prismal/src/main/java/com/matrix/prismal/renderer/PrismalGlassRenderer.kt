...
internal class PrismalGlassRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(glUnused: GL10?, config: EGLConfig?) {
        ...
        Log.d(TAG, "GL ready: bg=$bgProgram glass=$glassProgram blurH=$blurHProgram blurV=$blurVProgram size=${glassWidth.toInt()}x${glassHeight.toInt()}")
    }
    ...
    override fun onDrawFrame(glUnused: GL10?) {
        if (!frameLogged) {
            frameLogged = true
            Log.d(TAG, "first frame: backgroundTexture=$backgroundTexture blurFboReady=$blurFboReady")
        }
    ...
    fun setBackgroundTexture(bitmap: Bitmap) {
        Log.d(TAG, "upload bg texture ${bitmap.width}x${bitmap.height}")
