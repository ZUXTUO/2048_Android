...
open class PrismalFrameLayout @JvmOverloads constructor(
    ...
    private val glSurface: PrismalEglTextureView = PrismalEglTextureView(context)
    ...
    /** 诊断用：开启后持续渲染（60fps），便于抓取渲染像素日志；生产环境应关闭。 */
    fun setDiagnosticRender(enabled: Boolean) {
        glSurface.setRenderMode(
            if (enabled) GLSurfaceView.RENDERMODE_CONTINUOUSLY else GLSurfaceView.RENDERMODE_WHEN_DIRTY,
        )
        if (enabled) glSurface.requestRender()
    }

    /**
     * 设置在真实背景捕获完成前的兜底背景纹理。
     * 若窗口背景尚未就绪或捕获失败，GL 端回退到该纹理，
     * 避免玻璃区域渲染为纯黑。后续 captureAndSetBackground 会覆盖它。
     */
    fun setFallbackBackground(bitmap: Bitmap) {
        glSurface.queueAndRender { renderer.setBackgroundTexture(bitmap) }
    }
    ...
    fun PrismalEglTextureView.queueAndRender(block: () -> Unit) {
        queueEvent { block() }
        requestRender()
    }
}